...
struct ApplicationCatalogTests {
    ...
    func `missing application roots produce an empty catalog`() {
        ...
        #expect(ApplicationCatalog.scan(roots: [missingRoot], excluding: nil).isEmpty)
    }
    ...
    func `scans nested application bundles and ignores other files`() throws {
        ...
        let results = ApplicationCatalog.scan(roots: [root], excluding: nil)
    ...
    func `hidden application bundles are ignored`() throws {
        ...
        #expect(ApplicationCatalog.scan(roots: [root], excluding: nil).map(\.name) == ["Visible"])
    }

    @Test
    func `the launcher excludes its own application bundle`() throws {
        let root = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: root) }
        ...
        let results = ApplicationCatalog.scan(roots: [root], excluding: launcher)

        #expect(results.map(\.name) == ["Other"])
    }
    ...
    func `overlapping roots do not duplicate an application`() throws {
        ...
        let results = ApplicationCatalog.scan(roots: [root, root], excluding: nil)
    ...
    func `application bundle identifier comes from its bundle information`() throws {
        ...
        let result = ApplicationCatalog.scan(roots: [root], excluding: nil).first

...
struct LauncherSettingsUserStoryTests {
    ...
    func `launch at login uses native registration and approval paths`() {
        ...
        #expect(LauncherLoginItemService.loginItemStatus(for: .notFound) == .disabled)
        #expect(
            LauncherLoginItemActionPolicy.action(
                enabling: true,
                status: LauncherLoginItemService.loginItemStatus(for: .notFound),
            ) == .register,
        )
    }
    ...
    func `launch at login explains enabled approval and error states`() {
        ...
        let unavailable = LauncherLoginItemPresentation.content(for: .unavailable)
        #expect(!unavailable.message.contains("location"))
    }

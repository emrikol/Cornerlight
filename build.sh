...
APPLE_DEVELOPMENT_IDENTITY="Apple Development: Derrick Tennant (8VA8WSH6G4)"
DEVELOPER_ID_IDENTITY="Developer ID Application: Derrick Tennant (3T9RX85H44)"
...
resolve_signing_identity() {
    ...
    local preferred="$APPLE_DEVELOPMENT_IDENTITY"
    if [[ "$PREFER_DEVELOPER_ID" == true ]]; then
        preferred="$DEVELOPER_ID_IDENTITY"
    fi

    if [[ "$identities" == *\"$preferred\"* ]]; then
        print -r -- "$preferred"
        ...
        fail "required signing identity is unavailable: $preferred; use an explicit --sign-identity or --allow-adhoc"
    fi
}
...
cp "$PROJECT_ROOT/Resources/AppIcon.icns" "$STAGED_BUNDLE/Contents/Resources/AppIcon.icns"
...
print -- "Signed with: $RESOLVED_SIGN_IDENTITY"
if [[ "$INSTALL" == true ]]; then
